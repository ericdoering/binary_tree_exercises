...
class BinaryTree {
  ...
  nextLarger(lowerBound) {
    ...
    while (queue.length){
      ...
      let higherBound = currentVal > lowerBound;
      let reassignClosest = currentVal < closest || closest === null;

      if(higherBound && reassignClosest){
        closest = currentVal;
      }
  ...
  areCousins(node1, node2) {
    ...
    function findLevelAndParent(searchedNode, currentNode, level = 0, data = {level: 0, parent: null}){
      if (data.parent) return data;
      if (currentNode.left === searchedNode || currentNode.right === searchedNode){
        data.level = level + 1;
        data.parent = currentNode;
      }
      if (currentNode.left) {
        findLevelAndParent(searchedNode, currentNode.left, level + 1, data);
      }
      if (currentNode.right) {
        findLevelAndParent(searchedNode, currentNode.right, level + 1, data);
      }
      return data
    }
